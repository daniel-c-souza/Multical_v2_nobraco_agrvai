...
from svr_experiment.svr_engine import MulticalEngine
...
def main():
    ...
    # --- 2. Model Parameters ---
    Selecao = 4       # Model type: 4 = SVR
    kmax = 25         # Now represents total parameter combinations in Grid (C x Gamma)
    # Note: Engine defines a 5x5 grid (25 steps). Setting kmax >= 25 will run all of them.
    ...
    use_ftest = False  # DISABLE F-test for SVR (Selects Global Minimum instead)
...
('x_cel_jao_cal.txt', 'jao_espectros.txt'),
    ...
    if result is None or result[0] is None:
        print("Engine run failed or returned None.")
        return

    RMSECV, RMSECV_conc, RMSEcal, RMSEcal_conc, RMSEtest, RMSEtest_conc = result
    
    model_map = {1: "PLS", 2: "SPA", 3: "PCR", 4: "SVR"}
